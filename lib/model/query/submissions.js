...
order by path asc, value asc`)
  ...
  // const getTotalRegistros = (formId) => ({ one }) =>{ return one(sql`  SELECT
  const getTotalRegistros = (formId) =>{ return sql`  SELECT
  f.permitidos,
	COUNT(*) as t
FROM
	formulario_permitidos f 
	INNER JOIN submissions s ON (s."formId" = f.form_id)
WHERE
	f.form_id = ${formId}
	AND s."deletedAt" IS NULL
GROUP BY f.permitidos;`};
