...
const createNew = (partial, form, deviceIdIn = null, userAgentIn = null) => ({ one, context }) => {
  console.log("Inserta valores formulario")
  const total = one(sql`
  select count(*) as t from submissions as s
    where s."formId"=${form.id} and "deletedAt" is null`)
  console.log("consulta");
  console.log(total);
    return reject(Problem.user.unexpectedValue({ field: 'limite consolta', value: "consulta", reason: form.id }));
...
order by path asc, value asc`)
  ...
  // const getTotalRegistros = (formId) => { return one(sql`
  const getTotalRegistros = (formId) => ({ one }) =>{ return one(sql`
select count(*) as t from submissions as s
  where s."formId"=${formId} and "deletedAt" is null`)};
...
where
  ...
  ${odataFilter(options.filter, odataToColumnMap)} and
