...
${page(options)}`;
// formularios sin limite
const _getSqlformsNull = () => sql`
SELECT f."id", fp.permitidos, f."xmlFormId" FROM formulario_permitidos fp 
RIGHT JOIN forms f ON (fp.form_id = f."id")
WHERE fp.permitidos IS NULL
AND f."deletedAt" IS NULL
`;
// listado de formularios
const _getSqlformsAll = () => sql`
SELECT f."id", fp.permitidos, f."xmlFormId" FROM formulario_permitidos fp 
RIGHT JOIN forms f ON (fp.form_id = f."id")
WHERE f."deletedAt" IS NULL
`;
// update limite
const _getSqlUpdateLimite = (id, limite) => sql`
update formulario_permitidos set permitidos=${limite} where "form_id"=${id}
`;

const _getSqlInsertLimite = (id, limite) => sql`
insert INTO formulario_permitidos (form_id, permitidos) VALUES (${id},${limite})
`;
// listado de usuarios consulta
const getAll = (options) => ({ all }) => all(_getSql(options)).then(map(_unjoin));
const getFormsAll = () => ({ all }) => all(_getSqlformsAll());
const getFormsNull = () => ({ all }) => all(_getSqlformsNull());
const getUpdateLimite = (id, limite) => ({ all }) => all(_getSqlUpdateLimite(id, limite));
const getInsertLimite = (id, limite) => ({ all }) => all(_getSqlInsertLimite(id, limite));
...
module.exports = {
  ...
  emailEverExisted,
  getFormsNull, getFormsAll, getUpdateLimite, getInsertLimite
};
