...
module.exports = (service, endpoint) => {
  ...
  const openRosaSubmission = (path, draft, getForm) => {
    ...
    const streamAttachment = async (Submissions,valor) => {
      console.log("llego");
      const blob = await Submissions.getTotalRegistros(3);
    ...
    Submission.fromXml(findMultipart(files).buffer)
        .then((partial) => getForm(auth, params, partial.xmlFormId, Forms, partial.def.version)
          ...
          .then((form) => {
            ...
            console.log(form.id);
            let totalBase = streamAttachment(Submissions,3);
  ...
  const streamAttachment2 = async (Submissions,valor) => {
    console.log("llego2");
    const blob = await Submissions.getTotalRegistros(3);
    let da =  JSON.stringify(blob);
    let ba = JSON.parse(da)
    console.log("aqui22 - "+ ba.t);
    return reject(Problem.user.unexpectedValue({ field: '22----', value: "limite---2", reason: valor }));
