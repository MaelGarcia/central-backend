      ...
      Submission.fromXml(findMultipart(files).buffer)
        .then((partial) => getForm(auth, params, partial.xmlFormId, Forms, partial.def.version)
          ...
          .then((form) => {
            ...
            let uno = 0;
            it('oooooo', async () => {
              const result = await Submissions.getTotalRegistros(form.id);
              let r =JSON.stringify(result)
              let p = JSON.parse(r)
              uno = p.t;
            });
            const totalInser = Promise.resolve( Submissions.getTotalRegistros(form.id));
