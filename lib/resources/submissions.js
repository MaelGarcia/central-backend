...
const submission = require('../data/submission');
...
module.exports = (service, endpoint) => {
  ...
  const openRosaSubmission = (path, draft, getForm) => {
    ...
    const multerUtf = (request, _, next, Submissions) => {
      console.log("pppppppppp");
      console.log(request.params);
      Submissions.getTotalRegistros(3).then((r) => {
        let da =  JSON.stringify(r);
        let ba = JSON.parse(da)
        console.log("aqui1 - "+ ba.t);
      })
    ...
    let val = 0;
    const streamAttachment = async (Submissions,valor) => {
      console.log("llego");
      const blob = await Submissions.getTotalRegistros(valor);
      let da =  JSON.stringify(blob);
      let ba = JSON.parse(da)
      console.log("aqui - "+ ba.t);
      val = ba.t;
      setTimeout(() => {
        console.log("paso un tiempo");
        return ba.t;
      }, 5000);
      // return ba.t;
      // return reject(Problem.user.unexpectedValue({ field: '----', value: "limite---2", reason: valor }));
  ...
  // streamAttachment();
    ...
    // Nonstandard REST; OpenRosa-specific API.
    service.post(path, multipart.any(), multerUtf, endpoint.openRosa(async({ Forms, Submissions, SubmissionAttachments }, { params, files, auth, query, userAgent }) => 
    Submission.fromXml(findMultipart(files).buffer)
        .then((partial) => getForm(auth, params, partial.xmlFormId, Forms, partial.def.version)
          ...
          .then((form) => {
            ...
            setTimeout(() => {
              let totalBase3 = streamAttachment(Submissions,4);
              console.log("save "+totalBase3);
              console.log("paso un tiempo2");
              
            }, 5000);
            let item = 0;    
            console.log("paso 1 archivo");
            console.log("Total alcanzado");
            console.log(form.id);
            let totalBase = streamAttachment(Submissions,4);
            console.log("save "+totalBase);
            let totalBase2 = streamAttachment2(Submissions,3);
            console.log("save3 "+totalBase2);
            
            
          //   const something = async(Submissions, id) => {
          //     let result = await Submissions.getTotalRegistros(id)
          ...
          // console.log(something);
            ...
            // const totalInser =  Submissions.getTotalRegistros(form.id, item);
            // totalInser.then((value) => {
            //   let da = JSON.stringify(value);
            //   let ba = JSON.parse(da)
            //   console.log("val2 "+ba.t);
            //   item = ba.t;
            //   console.log("item = "+ ba.t);
            //   // return reject(Problem.user.unexpectedValue({ field: '----', value: "limite---2", reason: form.id }));
            // })
            // console.log("salida " + item);
                    
            // Submissions.getTotalRegistros(form.id, item).then((r) => {
            //   let datos = JSON.stringify(r);
            //   let base = JSON.parse(datos)
            //   console.log("data:: "+datos);
            //   //console.log("base:: "+base);
            //   console.log("valor = "+base.t);
            //   item  = base.t
            //   return reject(Problem.user.valueTooLong());              
            // });
            // console.log("item salida"+item);
            // let totalPermitidos = 1;
            // if (totalPermitidos == 1)
           
            console.log("valorllll :: "+val);
             return reject(Problem.user.unexpectedValue({ field: 'limite alcanzado_2', value: "limite2", reason: form.id }));
  ...
  const streamAttachment2 = (Submissions) => {
    console.log("llego2");
    const blob =  Submissions.getTotalRegistros(4).then((r) => {
      let da =  JSON.stringify(r);
      let ba = JSON.parse(da)
      console.log("da - "+ da );
      console.log("da - "+ da.t );
      console.log("ba - "+ ba );
      console.log("aqui22 - "+ ba.t);
      return resolve(ba.t);
    });
  ...
  const dataOutputs = (base, draft, getForm) => {
    ...
    service.get(`${base}/:instanceId/attachments/:name`, endpoint(({ Forms, Submissions, SubmissionAttachments }, context) =>
      getOrRedirect(Forms, Submissions, context)
    ...
    service.get(`${base}/:rootId/versions/:instanceId/attachments/:name`, endpoint(({ Forms, Submissions, SubmissionAttachments }, { params, auth }) =>
      getForm(params, Forms)
        .then(auth.canOrReject('submission.read'))
        .then((form) => Promise.all([
