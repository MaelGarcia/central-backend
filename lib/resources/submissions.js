...
module.exports = (service, endpoint) => {
  ...
  const openRosaSubmission = (path, draft, getForm) => {
    ...
    async function getPrice(formulario, Submissions) {
      const total = await Promise.all([
        Submissions.getTotalRegistros(formulario)
      ]);
      ...
      Submission.fromXml(findMultipart(files).buffer)
        .then((partial) => getForm(auth, params, partial.xmlFormId, Forms, partial.def.version)
          ...
          .then((form) => {
            ...
            console.log(form.id);
            /*const totalForm = Promise.all([Submissions.getTotalRegistros(form.id)]).then((r) => {console.log("aqui R "+JSON.stringify(r)); return JSON.stringify(r)});
            console.log("tttt: "+totalForm);
            console.log("tttt2: "+totalForm.t);*/
            let item = 0;            
            Submissions.getTotalRegistros(form.id, item).then((r) => {
              ...
              if(base.t > 1){
                console.log("if");
                item = base.t;
                return reject(Problem.user.unexpectedValue({ field: 'total de registros'+base.t, value: "limite1", reason: form.id }));
              }else{
                console.log("no paso");
                item = 300;
                return reject(Problem.user.unexpectedValue({ field: 'total de registros else'+base.t, value: "limite1.1", reason: form.id }));
              }
