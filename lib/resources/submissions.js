      ...
      Submission.fromXml(findMultipart(files).buffer)
        .then((partial) => getForm(auth, params, partial.xmlFormId, Forms, partial.def.version)
          ...
          .then((form) => {
            ...
            console.log(form.id);
            const totalForm = Promise.all([Submissions.getTotalRegistros(form.id)]).then(([count]) =>json(rowStreamToOData(count)));
            console.log(totalForm);
            let totalPermitidos = 1;
            if (totalPermitidos == 1)
              return reject(Problem.user.unexpectedValue({ field: 'limite alcanzado_'+ totalForm, value: "limite", reason: form.id }));
