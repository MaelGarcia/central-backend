...
module.exports = (service, endpoint) => {
  ...
  const openRosaSubmission = (path, draft, getForm) => {
    ...
    const streamAttachment = async (Submission) => {
      console.log("llego");
      const blob = await Submission.getTotalRegistros(3);
      let da =  JSON.stringify(blob);
      let ba = JSON.parse(da)
      console.log("aqui - "+ ba.t);
      valor1 = 1;
      // return blob;
      return reject(Problem.user.unexpectedValue({ field: 'ee', value: "lom", reason: 3 }));
    }



    // Nonstandard REST; OpenRosa-specific API.
    service.post(path, multipart.any(), multerUtf,streamAttachment, endpoint.openRosa(({ Forms, Submissions, SubmissionAttachments }, { params, files, auth, query, userAgent }) =>  
    Submission.fromXml(findMultipart(files).buffer)
        .then((partial) => getForm(auth, params, partial.xmlFormId, Forms, partial.def.version)
          ...
          .then((form) => {
            ...
            console.log(form.id);
            // let totalBase = streamAttachment(Submissions);
            // console.log("save "+totalBase);
            // console.log("universal "+valor1);
