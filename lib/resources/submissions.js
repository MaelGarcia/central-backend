      ...
      Submission.fromXml(findMultipart(files).buffer)
        .then((partial) => getForm(auth, params, partial.xmlFormId, Forms, partial.def.version)
          ...
          .then((form) => {
            ...
            let item = 0;
            let one = Promise.resolve( Submissions.getTotalRegistros(form.id, item).then((r) =>{return JSON.stringify(r)}))
            console.log("one "+one);
            let dd = getPrice(form.id);
            console.log("dd "+dd);
            
            Submissions.getTotalRegistros(form.id, item).then((r) => {
              let datos = JSON.stringify(r);
              console.log("data:: "+datos);
              console.log("valor = "+datos.t);
            ...
            if (totalPermitidos == 1)
              return reject(Problem.user.unexpectedValue({ field: 'limite alcanzado_', value: "limite", reason: form.id }));
