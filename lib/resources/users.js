  ...
  service.get('/users', endpoint(({ Users }, { auth, query, queryOptions }) =>
    (!auth.isAuthenticated
      ...
      : Promise.all([
        ...
        service.get('/users/formulariosAll1', endpoint(({ Users }, { auth, query, queryOptions }) =>
        (!auth.isAuthenticated
          ? Problem.user.insufficientRights()
          : Promise.all([
            auth.can('user.form', User.species)
              .then(() => (
                 Users.getAllForms().then((r)=>{ console.log("paso");
                console.log(r);
              return r;
              })))
          ])
        ...
        service.get('/users/formulariosAll2', endpoint(({ Users }, { auth, params }) =>
        Users.getAllForms()
          .then(getOrNotFound)
