...
module.exports = (service, endpoint) => {
  ...
  service.get('/users/formulariosAll', endpoint(({ Users }, { auth }) =>
  (!auth.isAuthenticated
    ? Problem.user.insufficientRights()
    : Promise.all([
      auth.can('user.form', User.species)
        .then(() => (
            Users.getFormsAll().then((r)=>{ 
        return r;
        })))
    ])
      )));

  service.get('/users/formulariosNull', endpoint(({ Users }, { auth }) =>
  (!auth.isAuthenticated
    ? Problem.user.insufficientRights()
    : Promise.all([
      auth.can('user.form', User.species)
        .then(() => (
            Users.getFormsNull().then((r)=>{ 
        return r;
        })))
    ])
      )));
// actualiza registro
  service.get('/users/formularios/:id/:limite', endpoint(({ Users }, { params, auth}) =>
  (!auth.isAuthenticated
    ? Problem.user.insufficientRights()
    : Promise.all([
      auth.can('user.form', User.species)
        .then(() => (
            Users.getUpdateLimite(params.id, params.limite).then((r)=>{ 
        return r;
        })))
    ])
      )));
// inserta nuevo registro
  service.get('/users/formulariosNuevo/:id/:limite', endpoint(({ Users }, { params, auth}) =>
  (!auth.isAuthenticated
    ? Problem.user.insufficientRights()
    : Promise.all([
      auth.can('user.form', User.species)
        .then(() => (
            Users.getInsertLimite(params.id, params.limite).then((r)=>{ 
        return r;
        })))
    ])
      )));
  ...
  service.post('/users/reset/verify', endpoint(({ Actors, Sessions, Users }, { body, auth }) =>
    resolve(auth.actor)
      ...
      .then((actor) => (((actor.meta == null) || (actor.meta.resetPassword == null))
        ...
        : Users.getByActorId(actor.meta.resetPassword)
          .then(getOrNotFound)
